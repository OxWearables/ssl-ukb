...
Original at https://github.com/OxWearables/biobankAccelerometerAnalysis/blob/master/accelerometer/summarisation.py
Modified to work with the custom data format and info file generated by inference.py
"""
...
def getActivitySummary(  # noqa: C901
    data: pd.DataFrame, info: pd.DataFrame, summary, labels,
    ...
    summary['file-startTime'] = pd.Series(utils.date_strftime(startTime), dtype='string')
    summary['file-endTime'] = pd.Series(utils.date_strftime(endTime), dtype='string')
    summary['file-firstDay(0=mon,6=sun)'] = pd.Series(startTime.weekday(), dtype='int32')

    # Quality checks
    summary['totalReads'] = pd.Series(info['NumTicks'], dtype='int32')
    summary['quality-goodCalibration'] = pd.Series(info['CalibOK'], dtype='int32')
    if summary['quality-goodCalibration'][0] > 0:
        summary['calibration-xOffset(g)'] = pd.Series(np.around(info['CalibxIntercept'], 4), dtype='float32')
        summary['calibration-yOffset(g)'] = pd.Series(np.around(info['CalibyIntercept'], 4), dtype='float32')
        summary['calibration-zOffset(g)'] = pd.Series(np.around(info['CalibzIntercept'], 4), dtype='float32')
        summary['calibration-xSlope'] = pd.Series(np.around(info['CalibxSlope'], 4), dtype='float32')
        summary['calibration-ySlope'] = pd.Series(np.around(info['CalibySlope'], 4), dtype='float32')
        summary['calibration-zSlope'] = pd.Series(np.around(info['CalibzSlope'], 4), dtype='float32')
        summary['calibration-errsBefore(mg)'] = pd.Series(np.around(info['CalibErrorBefore(mg)'], 4), dtype='float32')
        summary['calibration-errsAfter(mg)'] = pd.Series(np.around(info['CalibErrorAfter(mg)'], 4), dtype='float32')
        summary['calibration-numStaticPoints'] = pd.Series(info['CalibNumSamples'], dtype='float32')
    else:
        summary['calibration-xOffset(g)'] = \
            summary['calibration-yOffset(g)'] = \
            summary['calibration-zOffset(g)'] = \
            summary['calibration-xSlope'] = \
            summary['calibration-ySlope'] = \
            summary['calibration-zSlope'] = \
            summary['calibration-errsBefore(mg)'] = \
            summary['calibration-errsAfter(mg)'] = \
            summary['calibration-numStaticPoints'] = pd.Series(0, dtype='int')
    ...
    # Resolve non-wear
    isGoodCoverage = not (data['missing']  # check there's at least some data for each hour pocket
                          .groupby(data['missing'].index.hour)
                          .all().any())
    wearTime = info['WearTime(days)'][0]
    nonWearTime = info['NonwearTime(days)'][0]
    nonWearLen = info['NumNonwearEpisodes'][0]
    isGoodWearTime = wearTime >= 3  # check there's at least 3 days of wear time

    summary['wearTime-numNonWearEpisodes(>1hr)'] = pd.Series(int(nonWearLen), dtype='int32')
    summary['wearTime-overall(days)'] = pd.Series(utils.formatNum(wearTime, 2), dtype='float32')
    summary['nonWearTime-overall(days)'] = pd.Series(utils.formatNum(nonWearTime, 2), dtype='float32')
    summary['quality-goodWearTime'] = pd.Series(int(isGoodCoverage and isGoodWearTime), dtype='int32')
    ...
    data_imputed = writeMovementSummaries(data, labels, summary, impute=imputation)

    # Return physical activity summary
    return data, data_imputed, labels
...
def resolveInterrupts(data, epochPeriod, summary):
    ...
    summary['errs-interrupts-num'] = pd.Series(len(gaps), dtype='int32')
    summary['errs-interrupt-mins'] = pd.Series(utils.formatNum(gaps.sum().total_seconds() / 60, 1), dtype='float32')
...
def imputeMissing(data: pd.DataFrame, extrapolate=True):
...
def writeMovementSummaries(data, labels, summary, impute=True):  # noqa: C901
